...
static const char * shortOptions = "i:b:f:t:u:o:n:";
...
static const char * usageOptions[] =
    { "[-i|--idletime]"
    , "[-b|--base]"
    , "[-f|--idlefile]"
    , "[-t|--timeoutfile]"
...
static void usage ( const char * name ) {
    ...
    while ( usageOptions[i] != NULL ) {
        fprintf ( stdout, " %s", usageOptions[i++] );
    }
...
static void switchOption ( Options * options, int choice, char ** argv ) {
   switch ( choice ) {
       case   0:
...
void getOptions ( Options * options, int argc, char ** argv ) {
   ...
   while ( c != -1 ) {
       ...
       switchOption ( options, choice, argv );
